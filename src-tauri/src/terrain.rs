...
#[tauri::command]
pub async fn load_dem_mesh(app: AppHandle, stride: u32) -> Result<Response, String> {
    let path = app
        .path()
        .resource_dir()
        .map_err(|e| format!("resource_dir: {}", e))?
    ...
    println!("[dem] reading {:?}, stride={}", path, stride);

    let bytes = tauri::async_runtime::spawn_blocking(move || -> Result<Vec<u8>, String> {
        let file = BufReader::new(File::open(&path).map_err(|e| format!("open: {}", e))?);
        let mut tiff = GeoTiffReader::open(file).map_err(|e| format!("parse: {}", e))?;

        let (img_w, img_h) = tiff
            .image_info()
            .dimensions
            .ok_or_else(|| "no dimensions".to_string())?;

        // Bounds from Copernicus DSM convention: origin is top-left (WGS84)
        // From your gdalinfo: Origin = (92.9998611, 25.0001389), Pixel Size = 0.0002777
        let west = 92.9998611_f64;
        let north = 25.0001389_f64;
        let px_size = 0.000277777777778_f64;
        let east = west + img_w as f64 * px_size;
        let south = north - img_h as f64 * px_size;

        let stride = stride.max(1);
        let out_w = (img_w + stride - 1) / stride;
        let out_h = (img_h + stride - 1) / stride;

        println!(
            "[dem] source {}×{}, output {}×{} ({:.1}M verts)",
            img_w,
            img_h,
            out_w,
            out_h,
            (out_w as f64 * out_h as f64) / 1e6
        );

        let mut heights: Vec<f32> = Vec::with_capacity((out_w * out_h) as usize);
        for j in 0..out_h {
            for i in 0..out_w {
                let px = (i * stride).min(img_w - 1);
                let py = (j * stride).min(img_h - 1);
                let v = tiff.read_pixel(px, py);
                let h = match v {
                    RasterValue::F32(x) => x,
                    RasterValue::F64(x) => x as f32,
                    RasterValue::I16(x) => x as f32,
                    RasterValue::I32(x) => x as f32,
                    RasterValue::U8(x) => x as f32,
                    RasterValue::U16(x) => x as f32,
        ...
        //   f64 west, f64 south, f64 east, f64 north
        //   f32 heights[row-major, north→south]
        let mut out = Vec::with_capacity(32 + heights.len() * 4);
        out.extend_from_slice(&out_w.to_le_bytes());
        out.extend_from_slice(&out_h.to_le_bytes());
